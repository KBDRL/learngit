...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        if a == b:
            return -1, 0
        while True:
            a_rem = self.names_to_nodes[a_rem].get_parent().__str__()
            if a_rem != 'None':
                t += 1
            if a_rem == 'a' or a_rem == "None":
                break
            if a_rem == b:
                return -1, t
        while True:
            b_rem = self.names_to_nodes[b_rem].get_parent().__str__()
            if b_rem != 'None':
                r += 1
            if b_rem == 'a' or b_rem == "None":
                break
            if b_rem == a:
                return -1, r

        return min(t, r) - 1, abs(t - r)
...
t, r = f.cousin("g", "c")
print "'g' is a", words[t], "cousin", r, "removed from 'c'"
if t == -1 and r == 1:
    print "yes"
else:
    print 'NO'
